        ...
        <div class="app-content-header">
            ...
            <div class="container">
                <div class="form-container">
                    ...
                    <form method="post" action="{{ route('adminvariants.update', $variant->id) }}">
                        @csrf
                        <div class="form-section">
                            <h2>Edit Variants</h2>
                            <div class="input-group">
                                <label for="categories">Categories</label>
                                <select id="cars" value="{{ $variant->attribute_id }}" name="attribute_id">
                                    @foreach ($attribute as $attributes)
                                        <option value="{{ $attributes->id }}">{{ $attributes->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" id="companyName" name="name" value="{{ $variant->name }}" required>
                                @error('name')
                            ...
                            <div class="input-group d-flex justify-content-start" style="margin-top: 20px;">
                                <button type="submit" class="btn btn-outline-warning btn-lg"
                                    style="margin-right: 10px;">Edit Variants</button>
                            </div>
                        </div>
                    </form>
